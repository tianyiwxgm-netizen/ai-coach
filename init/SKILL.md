...
name: ai-coach-init
version: 4.0.0
description: "AI Coach 环境自检与初始化：版本检查 + 三层检查（全局/角色/日常），静默修复，确保工具链就绪"
---
...
1. 执行 `curl -sf --max-time 3 https://raw.githubusercontent.com/tianyiwxgm-netizen/ai-coach/main/SKILL.md | head -5`
2. 从返回内容提取 `version:` 字段值
3. 与本地 `SKILL.md` 的 `version:` 比较
4. **有新版本时**：输出一行提醒 — "有新版本 {remote}（当前 {local}），运行 install.sh 可更新"
5. **无更新 / 网络失败 / 超时**：不输出任何内容
